...
//! \file
//! \brief A Sphere is a type of prim; derives from Prim
...
void Sphere::GetCreateSQLFromXMLEx( TiXmlElement *pElement, char *SQL )
{
    //    Debug( "Cube::GetCreateSQLFromXMLEx\n" );

    sprintf( SQL, "insert into spheres (i_reference, color_0_r,color_0_g,color_0_b)select %i,color_0_r,color_0_g,color_0_b from spheres where i_reference = 0;",
             atoi( pElement->Attribute( "ireference" ) ) );

    Prim::GetCreateSQLFromXMLEx( pElement, SQL );
}
void Sphere::GetUpdateSQLFromXMLEx( TiXmlElement *pElement, char *SQL )
{
    ...
    {
        sprintf( SQL, "update spheres set color_0_r=%f, color_0_g=%f, color_0_b=%f where i_reference = %i;",
    ...
    {
        sprintf( SQL, "%s update spheres set s_texture_reference='%s' where i_reference = %i;",
                 SQL,
...
char *Sphere::GetWorldStateRetrieveSQL()
{
    return   "SELECT a.i_reference,s_object_name,a.s_object_type,b.s_prim_type,a.i_owner,i_parentreference,"
             "pos_x,pos_y,pos_z,rot_x,rot_y,rot_z,rot_s,scale_x,scale_y,scale_z,s_texture_reference,s_script_reference,b_physics_enabled,b_phantom_enabled,b_terrain_enabled,b_gravity_enabled,"
             "color_0_r,color_0_g,color_0_b FROM objects a, prims b, spheres c "
             "where a.i_reference=b.i_reference and a.i_reference=c.i_reference and "
             "a.s_object_type='Prim' and b.s_prim_type='Sphere';";
}
void Sphere::PopulateFromDBRow()
{
...
void Sphere::UpdateFromXML( TiXmlElement *pElement )
{
...
void Sphere::LoadFromXML( TiXmlElement *pElement )
{
...
void Sphere::WriteToXMLDoc( TiXmlElement *pElement )
{
    //  Debug( "Cube::WriteToXMLDoc\n" );
    pElement->SetAttribute( "type", "SPHERE" );
...
//void Sphere::CopyTo( Sphere *ptargetsphere )
//{
//ptargetsphere->color0 = color0;
//strcpy( ptargetsphere->sTextureReference, sTextureReference );
//Prim::CopyTo( ptargetsphere );
//}
const Sphere &Sphere::operator=( const Sphere &IncomingPrim )
{
...
void Sphere::Draw()
{
    ...
    pmvGraphics->DoSphere();
    pmvGraphics->Bind2DTexture( 0 );

    pmvGraphics->PopMatrix();

    //Debug( "SPHERE"::Draw done\n" );
    Prim::Draw();
}
void Sphere::DrawSelected()
{
    float mcolor[4];
    ...
    pmvGraphics->Scalef( scale.x, scale.y, scale.z );
    pmvGraphics->DoWireSphere();

    pmvGraphics->PopMatrix();

    //      Debug( "SPHERE"::DrawEx done\n" );
    Prim::Draw();
}

void Sphere::WriteToDeepXML( TiXmlElement *pElement )
{
...
void Sphere::SetColor( int face, const Color &newcolor )
{
    color0 = newcolor;
}

void Sphere::SetTexture( int face, const char *sTextureReference )
{
    sprintf( this->sTextureReference, sTextureReference );
}

Color &Sphere::GetColor( int face )
{
    return color0;
}

const char *Sphere::GetTexture( int face )
{
